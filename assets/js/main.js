class Character {
  ...
  constructor(race, item) {
    ...
    this.heal = function () {
      ...
      let totalHeal = Math.round(
        randFromRange([this.minHealing, this.maxHealing]) * this.boostHealing
      );
    ...
    this.setRaceBoost = function () {
      switch (this.race) {
        case 'student':
          this.maxDamageTaken = this.maxDamageTaken * 0.8;
          break;
        case 'nick':
...
function randFromRange(range) {
  return Math.round(Math.random() * (range[1] - range[0]) + range[0]); //random between min - max included
}
...
function game() {
  ...
  function setLife(playerAttack, playerDefense) {
    let percentP1 = Math.round(
      (playerAttack.currenthealth / playerAttack.maxHealth) * 100
    );
    let percentP2 = Math.round(
      (playerDefense.currenthealth / playerDefense.maxHealth) * 100
    );
  ...
  function endTurnP1() {
    ...
    if (player2.race == 'nico') {
      ...
      player1.currenthealth =
        player1.currenthealth -
        Math.round(player1.currenthealth * player2.lifeSteal);
      player2.currenthealth =
        player2.currenthealth +
        Math.round(player1.currenthealth * player2.lifeSteal);
      console.log(
        'vie volée = ' + Math.round(player1.currenthealth * player2.lifeSteal)
      );
      addConsole(
        'vie volée par ' +
          player2.name +
      ...
      addConsole(
        'vie de ' + player1.name + ' debut du tour : ' + player1.currenthealth
      );
      console.log('vie JOUEUR 2 debut du tour : ' + player2.currenthealth);
      addConsole(
        'vie ' + player2.name + ' debut du tour : ' + player2.currenthealth
      );
      setLife(player1, player2);
    }
  }
  function endTurnP2() {
    ...
    if (player1.race == 'nico') {
      ...
      player2.currenthealth =
        player2.currenthealth -
        Math.round(player2.currenthealth * player1.lifeSteal);
      player1.currenthealth =
        player1.currenthealth +
        Math.round(player2.currenthealth * player1.lifeSteal);
      console.log(
        'vie volée = ' + Math.round(player2.currenthealth * player1.lifeSteal)
      );
      addConsole(
        'vie volée = ' + Math.round(player2.currenthealth * player1.lifeSteal)
      );
      ...
      console.log('vie JOUEUR 2 debut du tour : ' + player2.currenthealth);
      addConsole(
        'vie' + playe2.name + 'debut du tour : ' + player2.currenthealth
      );
      console.log('vie JOUEUR 1 debut du tour : ' + player1.currenthealth);
      addConsole(
        'vie ' + player1.name + 'debut du tour : ' + player1.currenthealth
      );
      setLife(player2, player1);
    }
  }
...
function switchIMG(i, idpic, idh2, idp) {
  ...
  switch (i) {
    ...
    case 2:
      ...
      CharInfo.innerHTML =
        '30% chance to reflect 50% of the attack back to the opponent.';
